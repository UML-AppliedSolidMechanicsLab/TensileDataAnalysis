...
//		or singular matrix, inconsistent system
		...
		for (i = 0; i<n;i++){
			scaleFactors[i, 0] = 0.0;
				for (j = 0; j<n;j++){
					if(System.Math.Abs(A[i,j])>scaleFactors[i,0]){
						scaleFactors[i,0] = System.Math.Abs(A[i,j]);
				}
			}
		}
		for (k = 0; k<n-1; k++){	// loop over columns of A
			...
			for (i= k; i<n; i++){	// loop over rows   '
				if(System.Math.Abs(A[i,k]/scaleFactors[i,0])>S){ 		// look for the
					S = System.Math.Abs(A[i,k]/scaleFactors[i,0]);		// maximum element
					imax = i;									// in column k                                        '
				}
			}
			if(System.Math.Abs(A[imax,k]) <= buffer){
				goto End_of_k_loop;
			}
		...
		while(i>=0){	// loop backwards over rows of A
        	...
        	R = B[i,C] - R;
			if(System.Math.Abs(A[i,i])>buffer){
				B[i,C] = R / A[i,i];
			}
			else if(System.Math.Abs(R) <=buffer) {	// A is singular
